...
func openStoreForTest(t *testing.T) *Store {
	t.Helper()
	store, err := Open(filepath.Join(t.TempDir(), "hooks.db"))
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
...
func TestBusyTimeoutIsSetOnEveryPooledConnection(t *testing.T) {
	for connectionIndex, value := range readPragmaFromEveryPooledConnection(t, openStoreForTest(t).DB(), "busy_timeout") {
...
func TestForeignKeysAreOnEveryPooledConnection(t *testing.T) {
	for connectionIndex, value := range readPragmaFromEveryPooledConnection(t, openStoreForTest(t).DB(), "foreign_keys") {
		if value != 1 {
...
func TestPerConnectionPragmaControl(t *testing.T) {
	// No DSN, exactly as hook-store opened its database before the fix.
